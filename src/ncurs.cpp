...
//     //                     left is font|right is background
...
//     running = true;
//     updateStatsThread = std::thread(&NcursesUI::updateStatsLoop, this);
...
// void NcursesUI::updateStatsLoop() {
//     std::lock_guard<std::mutex> lock(mtx);
//     while (running) {
//         getCpuStats(&total, percore, sizeof(total), sizeof(percore));
//     }
// }

// void fillTotalCpuUsage(float &total, char *buffer, int size){
//     snprintf(buffer, size, "%.2f", total);
// }
...
//     // mvwprintw(main_win, 3, 12,"%s%%", totalCPUUsageBuffer); // placeholder for total CPU usage

//     // CPU per-core
//     wattron(main_win, COLOR_PAIR(TEXT_COLOR));
//     mvwprintw(main_win, 9, 1, " Per core usage: ");
//     for (int i = 0; i < (int)corecount; i++) {
//         float usage = percore[i];
//         wattron(main_win, COLOR_PAIR(CPU_TEXT_COLOR));
//         mvwprintw(main_win, 10 + i, 2, "CPU%-2d:", i);
...
//         wprintw(main_win, "] ");
//         wattron(main_win, COLOR_PAIR(CPU_TEXT_COLOR));
//         wprintw(main_win, "%2.1f%%", usage);
//     }

//     // Memory Usage
//     wattron(main_win, COLOR_PAIR(TEXT_COLOR));
//     mvwprintw(main_win, 3, 2, "Memory Usage:");
...
//     wattron(main_win, COLOR_PAIR(TEXT_COLOR));
//     mvwprintw(main_win, 6, 2, "Uptime: %02d:%02d:%02d",
//              uptime/3600, (uptime%3600)/60, uptime%60);
//     mvwprintw(main_win, 7, 2, "Tasks: %d, %d running", process_count, 2);
//     mvwprintw(main_win, 7, 2, "Load avg: 0.15, 0.30, 0.25");
// }
...
//     snprintf(totalCPUUsageBuffer, sizeof(totalCPUUsageBuffer), "CPU%% %.2f ", total);
...
//         wattron(main_win, COLOR_PAIR(CPU_TEXT_COLOR));
//         wprintw(main_win, "%5.1f ", p.getCpuUsage());
//         wattron(main_win, COLOR_PAIR(MEM_TEXT_COLOR));
//         wprintw(main_win, "%10.1f", p.getMemUsage());
